...
const navMenu = [
    { icon: <Book className="size-5" />, text: "Dashboard", href: "/" },
    { icon: <MessageSquare className="size-5" />, text: "Blogs", href: "/blogs" },
    { icon: <Edit className="size-5" />, text: "Add Blog", href: "/add-blog" },
];

function Sidebar({ open, isMinimized, toggleMinimized, toggleSidebar }: {
    open: boolean;
    isMinimized: boolean;
    ...
    return (
        <aside
            id="app-menu"
            className={`fixed inset-y-0 left-0 z-50 bg-blue-200 transition-all duration-300 transform ${open ? "translate-x-0" : "-translate-x-full"
                } ${isMinimized ? "w-20" : "w-72"
                } lg:translate-x-0 lg:static flex flex-col h-screen`}
        >
            <div className={`flex items-center justify-between ${isMinimized ? "flex-col" : "flex-row"}`}>
                <div className="sticky top-0 flex h-16 items-center justify-start px-6">
                    <a href="/">
                        <img src={isMinimized ? logoSm : logo} alt="logo" className={isMinimized ? "h-8" : "h-10"} />
                    </a>
                </div>
            ...
            <nav className={`p-4 overflow-y-auto flex-1 ${isMinimized ? 'px-2' : 'lg:ps-8'}`}>
            ...
            <div className="mt-auto p-4 ">
                <Button
                    variant="ghost"
                    className={`w-full hover:cursor-pointer ${isMinimized ? "justify-center px-0" : "justify-start gap-2"
